...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Enforce casher email hard-coded
        if (role === 'cashier2') {
        ...
        setEmail(role === 'cashier2' ? 'syedabuzarzaidi07@gmail.com' : "");
      }
    } else {
      setEmail(role === 'cashier2' ? 'syedabuzarzaidi07@gmail.com' : "");
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Enforce cashier2 login restriction
      if (role === 'cashier2' && email.toLowerCase() !== 'syedabuzarzaidi07@gmail.com') {
        toast.error('Only the assigned Casher account can log in here');
        setLoading(false);
        return;
      }
        ...
        <Card className="border-none shadow-xl bg-white/85 backdrop-blur-md">
          <CardHeader className="space-y-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <CardTitle className="text-2xl font-black font-heading uppercase tracking-tight">Login as {role === 'cashier' ? 'Anas' : (role === 'cashier2' ? cashier2Label : role.charAt(0).toUpperCase() + role.slice(1))}</CardTitle>
